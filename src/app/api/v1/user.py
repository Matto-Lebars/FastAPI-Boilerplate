...
async def write_user(request: Request, user: UserCreate, db: Annotated[AsyncSession, Depends(get_db)]) -> UserRead:
    ...
    created_user = await crud_user.create(db=db, obj_in=user_internal)
...
async def read_users(
    request: Request, db: Annotated[AsyncSession, Depends(get_db)], page: int = 1, items_per_page: int = 10
) -> dict:
    ...
    total_count = await crud_user.count(db=db, filters={"is_deleted": False})

    response: dict[str, Any] = paginated_response(
        crud_data=users_data, total_count=total_count, page=page, items_per_page=items_per_page
    )
    return response


@router.get("/user/me/", response_model=UserRead)
async def read_user_me(request: Request, current_user: Annotated[dict, Depends(get_current_user)]) -> dict:
    return current_user
...
async def patch_user(
    ...
    db_user = await crud_user.get(db=db, username=username)
    if db_user is None:
        raise NotFoundException("User not found")
    ...
    if values.email is not None and values.email != db_email:
        if await crud_user.exists(db=db, email=values.email):
            raise DuplicateValueException("Email is already registered")

    if values.username is not None and values.username != db_username:
        if await crud_user.exists(db=db, username=values.username):
            raise DuplicateValueException("Username not available")

    await crud_user.update(db=db, db_obj=db_user, obj_in=values)
...
async def erase_user(
    ...
    await crud_user.soft_delete(db=db, db_obj=db_user)
...
async def erase_db_user(
    ...
    await crud_user.hard_delete(db=db, db_obj=db_user)
    await blacklist_token(token=token, db=db)
